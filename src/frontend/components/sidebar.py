...
def render_sidebar() -> None:
    ...
    with st.sidebar:
        st.markdown('<div class="mc-app-title">🩺 Medical RAG</div>', unsafe_allow_html=True)
        st.caption("Chatbot médical intelligent — contexte marocain")
        st.divider()
        ...
        st.divider()
        st.caption("Modèle (à venir — Personne 2)")
        st.selectbox(
            "Modèle LLM",
        ...
        st.divider()
        st.caption("⚠️ Cet outil ne remplace pas un avis médical professionnel.")
...
def _render_export_button() -> None:
    ...
    if not messages:
        st.button("⬇️ Exporter", use_container_width=True, disabled=True)
        return
